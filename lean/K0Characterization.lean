...
theorem k0_witness_iff (x₀ : ℕ) (hx : 0 < x₀) (hx_mod : x₀ % 3 = 1) :
  ...
  · intro hw
    ...
    have hd_mod : d % 3 = 2 := by
      ...
      ·
        have hcontr : ((1 + 0) % 3) ≠ 0 := by decide
        have : (1 + 0) % 3 = 0 := by simpa [hrd] using hsum''
        exact (False.elim (hcontr this))
      ·
        have hcontr : ((1 + 1) % 3) ≠ 0 := by decide
        have : (1 + 1) % 3 = 0 := by simpa [hrd] using hsum''
        exact (False.elim (hcontr this))
      ·
        exact hrd
    -- If there were no prime divisor of `x₀` with residue 2, then all prime divisors of `x₀` would be 1 mod 3,
    -- forcing every divisor of `x₀^2` (including d) to be 1 mod 3, contradicting `hd_mod`.
    by_contra hP
    have hall : ∀ p, Nat.Prime p → p ∣ x₀ → p % 3 = 1 := by
      intro p hp hpx
      have hlt : p % 3 < 3 := Nat.mod_lt p (by decide)
      interval_cases hpm : p % 3 using hlt
      ·
        have h3p : 3 ∣ p := Nat.dvd_of_mod_eq_zero hpm
        have h3x : 3 ∣ x₀ := dvd_trans h3p hpx
        have hx0 : x₀ % 3 = 0 := Nat.mod_eq_zero_of_dvd h3x
        have h10 : (1 : ℕ) = 0 := by simpa [hx_mod] using hx0
        exact (False.elim (Nat.one_ne_zero h10))
      ·
        exact hpm
      ·
        have : False := hP ⟨p, hp, hpx, hpm⟩
        exact this.elim
    have hd1 : d % 3 = 1 := divisors_mod3_of_primes_mod3 x₀ hx hall d hd_dvd
    have h21 : (2 : ℕ) = 1 := by simpa [hd_mod] using hd1
    exact (by decide : (2 : ℕ) ≠ 1) h21
  · intro hpExists
    rcases hpExists with ⟨p, hpPrime, hpDiv, hpMod⟩
    have hp_le : p ≤ x₀ := Nat.le_of_dvd hx hpDiv
    refine ⟨p, ?_, hp_le, ?_⟩
